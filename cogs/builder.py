...
class OwnerReviewView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        update_builder_case_status(self.case_id, "approved")
        log_builder_timer_event(self.case_id, "approved", interaction.user.id,
                                f"Payment approved by {interaction.user}")
        await self._notify_parties(interaction, "approved", "✅ Payment Approved",
                                   discord.Color.green())
        self._disable_all()
        await interaction.message.edit(view=self)

    @discord.ui.button(label="❌ Hold Payment", style=discord.ButtonStyle.danger)
    async def hold(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        update_builder_case_status(self.case_id, "on_hold")
        log_builder_timer_event(self.case_id, "on_hold", interaction.user.id,
                                f"Payment held by {interaction.user}")
        await self._notify_parties(interaction, "on_hold", "⏸️ Payment On Hold",
                                   discord.Color.orange())
        self._disable_all()
        await interaction.message.edit(view=self)

    @discord.ui.button(label="⚠️ Investigation Required", style=discord.ButtonStyle.secondary)
    async def investigate(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        update_builder_case_status(self.case_id, "under_investigation")
        log_builder_timer_event(self.case_id, "under_investigation", interaction.user.id,
        ...
        embed = discord.Embed(
            ...
            description=(
                f"Case `{self.case_id}` has been flagged for investigation.\n"
        ...
        await self._notify_parties(interaction, "under_investigation",
                                   "⚠️ Investigation Required", discord.Color.yellow())
        self._disable_all()
        await interaction.message.edit(view=self)

    async def _notify_parties(self, interaction: discord.Interaction,
                               status: str, title: str, color: discord.Color):
        case = get_builder_case(self.case_id)
        if not case:
            return
